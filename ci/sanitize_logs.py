...
RULES = [
    ...
    (re.compile(r"(?i)authorization:\s*bearer\s+[A-Za-z0-9\-\._~\+\/=]+"), "Authorization: [REDACTED]"),
    (re.compile(r"(?i)authorization:\s*[^\s]+"), "Authorization: [REDACTED]"),
    # Common secret patterns like API_KEY=xxx or SECRET: 'xxx'
    (re.compile(r"(?i)(api[_-]?key|access[_-]?token|secret|password|passwd)[\s'\"]*[:=][\s'\"]*[^\s'\"]+"), "\1: [REDACTED]"),
    # Credentials in URLs
...
def main():
    if len(sys.argv) < 3:
        print("Usage: ci/sanitize_logs.py input.txt output-sanitized.txt", file=sys.stderr)
        sys.exit(2)
    inp = sys.argv[1]
    out = sys.argv[2]
